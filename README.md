...
## Плановые подарки (по дате)

Можно настроить файл `plugins/joupen/scheduled-gifts.txt`, который проверяется автоматически в полночь.

Формат строки:
`<nick> <duration> <yyyy-MM-dd>`

Примеры:
- `PlayerOne 30d 2026-03-10`
- `PlayerTwo 2mo5d 2026-03-15`

Правила:
- в полночь выдаются все записи, где дата меньше или равна текущей;
- выданные записи удаляются из файла;
- при ошибке выдачи строка остаётся в файле и будет повторно обработана следующей ночью;
- это подарочное продление, оно изменяет только срок действия проходки.
